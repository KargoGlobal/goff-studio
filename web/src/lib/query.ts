...
export function describeGroup(group: RuleGroupType): string {
  const parts: string[] = []
  ...
  for (const child of group.rules) {
    ...
    if ('rules' in child) {
      const nested = describeGroup(child)
      if (nested) {
        const wrapped = child.rules.length > 1 ? `(${nested})` : nested
        parts.push(child.not ? `not ${wrapped}` : wrapped)
      }
      continue
    }

    const attribute = (child.field ?? '').trim()
    if (!attribute) continue

    const arity = arityOf(child.operator)
    if (arity === 0) {
      parts.push(`${attribute} ${labelOf(child.operator)}`)
      continue
    }

    const raw = child.value === undefined || child.value === null ? '' : String(child.value)
    if (raw.trim() === '') continue

    if (arity === 2) {
      const items = raw.split(',').map((v) => v.trim()).filter(Boolean)
      const negated = child.operator === 'notin'

      if (items.length === 1) {
        parts.push(`${attribute} ${negated ? 'does not equal' : 'equals'} ${items[0]}`)
        continue
      }

      const readable = `${items.slice(0, -1).join(', ')} or ${items.at(-1)}`
      parts.push(`${attribute} ${negated ? 'is not one of' : 'is one of'} ${readable}`)
      continue
    }

    parts.push(`${attribute} ${labelOf(child.operator)} ${raw}`)
  }

  if (parts.length === 0) return ''

  const joined = parts.join(group.combinator === 'or' ? ' or ' : ' and ')
  if (!group.not) return joined
