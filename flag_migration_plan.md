...
Status: draft, 2026-09-25. Owner: SSP / MLE platform.

This document assesses what Kargo's feature-flag and experimentation usage requires of its
...
- **39 flags have not been edited in over a year.** Some are still read by code (for example
  `kraken_ctr_model`), so "untouched" does not mean "unused". The import keeps them all and
  the stale-flag report (F20) sorts them out afterwards.
- **Seven SDK keys, one per service per environment**: kraken, scylla (two in production) and
...
| --- | --- | --- | --- | --- |
| **kraken** (bidder) | Go | biddableID, adSlotID, bidID or dealID, chosen per flag | bool, numeric, string, JSON | none (logger is a no-op) |
| **scylla** (exchange) | Go | scyllaImpID, sometimes request ID | bool, numeric, integer, string, JSON | every assignment, as experiment events to Snowflake |
| **fvp-debugger-tool / fvp-ad-manager** (video player loader) | browser JS | random per page load | JSON | vendor-side |
...
`GetBool / GetFloat64 / GetString / GetJSON` (plus `GetInt64` in scylla) that fall back to a
default value on error. **That interface is the migration seam.** Only the
implementation behind it changes. Call sites don't.

Targeting attributes in use: publisherID, biddableID, biddableType, campaignID, dealID,
...
2. **Per-entity overrides.** One rule per ID, each serving its own variation, then a default.
   For example, `bidding-strategy` has 14 production rules of the form "biddableID is one of X,
...
- **Mostly configuration and kill switches, some A/B.** Most kraken flags are rollout gates or
  JSON config (bidding strategy, frequency caps, supply shaping, OpenRTB field overrides).
- **ML model routing.** kraken's current model-routing loader reads a JSON flag of the form
...
- **Experiments with warehouse analysis, now dormant.** No analysis has started since
  February 2026. scylla's assignments still land in
  the Snowflake assignment table, and an arion DAG joins them to outcomes every 3 hours in
...
- **Per-flag env-var master switches** exist in scylla and kraken to limit metered
...
- **Teams exist but don't restrict editing.** Kraken (about 37 members), Fabrik Player,
  Brain and a few small ones. Team only labels ownership, and most JSON flags have no team
...
- **A workspace-level Slack app posts to two channels**, a flag-notifications channel and
  `#ssp-deployments`. For example: "Feature Flag *tmax* was turned *OFF* by *&lt;user&gt;*."
...
```
  Okta (OIDC + groups claim)
          │
   GO Feature Flag Studio ── Flags · Experiments · Models ──commit as GitHub App──►  KargoGlobal/<private flags repo>
          ▲                                                                            staging/*.goff.yaml
          │ reads results                                                              production/*.goff.yaml
          │                                                                            experiments/*.yaml   metrics/*.yaml
          │                                                                                    │ GitHub Action on push
          │                                                                                    ▼
          │                                                                            S3 bucket (per env prefix)
          │                                                                                    │ 60s poll
          │                                                     ┌──────────────────────────────┴──────────────┐
          │                                                   kraken                                      scylla
          │                                     (shared kargo-flags package: GOFF client + split evaluator)
...
- **Flags live in a new private repo, not this one.** This repo is a public fork. The flags
  repo gets branch protection so only Studio's GitHub App can push to `main`. Every change is
  then a commit with the signed-in user as author, which gives history, blame and rollback
  for free. Experiment definitions and the metric catalog live in the same repo, so they get
  the same review and history.
- **One shared Go package evaluates flags everywhere.** kraken, scylla and Studio's preview
  panel all import the same `kargo-flags` package, so a subject gets the same answer in every
...
- **Services read from S3, not straight from GitHub.** Hundreds of pods polling every 60 seconds
  would exhaust GitHub API rate limits. A one-step GitHub Action (`aws s3 sync`) on push to
  `main` avoids that, and the pods already have AWS credentials through IRSA.
...
- **The statistics kernel is adopted, not written.** GrowthBook's `gbstats` package
  ...
  sufficient statistics as input, which fits the table design exactly. `statsmodels`
  (BSD) adds multiple-comparison corrections and stratum regression.
- **Studio is the operational surface and Omni is the exploration surface.** Studio shows
  set-up, live monitoring and the decision view for one experiment. Omni holds the semantic
...
- **Randomization unit.** Each experiment declares `unit: request` (impression, bid or request
  IDs) or `unit: entity` (ad slot, biddable, deal) and the attribute that carries it. Stickiness
  and ramp safety only mean something for entity units, and Studio says so.
- **Stratified assignment for entity units.** For biddable or deal tests, Studio can generate a
...
- **No model (CTR, VCR, VSR) or CTV attention test has ever been set up as a formal analysis.**
  Those are judged in Omni and Grafana.
- **Two entity types:** `bid_id` for exchange and bidder tests, and `video_player` for the
  video-player tests. The video-player tests read Fabrik impression tables.
...
- **One metric source's incremental aggregate hits the 4-hour timeout.** Snowflake query logs
  in Monte Carlo show it timing out daily from September 21 to 24, on a Medium warehouse,
...
- **The assignment load drops exposures.** It dedupes on the subject only, both within a batch
  and against the last 12 hours (the assignment insert in arion's `auction_flow` ingest, current `master`).
...
- **Kraken logs no assignments at all,** and its model predictor events carry no bid ID, so
  kraken model tests cannot be joined to outcomes through this pipeline.
- **In practice decisions are made by ramping and watching.** A product manager posts the
  flag, an Omni dashboard and a Grafana link, and ramps 1% → 5% → … → 100% over days or weeks.
...
All tables live in a new `exp` schema and are built by arion.
...
| `exp_results` | (experiment, metric, variant, as-of time) | Lift, CI, p-value, adjusted and raw, SRM and guardrail status. Read by Studio and Omni. |
| `flag_change_log` | commit | Who changed which flag, when, before and after. Built from the flags repo. Used for chart annotations and incident timelines. |
...
- **Squares only add up across days when each unit lives in one day.** That holds for
  per-impression units. For ad slots, biddables and deals, keep a cumulative per-unit table and
...
- **Clustered designs.** When the analysis is per impression but randomization is per
  biddable or deal, the metric is a ratio of per-cluster sums with n = number of clusters.
...
- **Fixed horizon or sequential.** The default is a fixed horizon in whole weeks, sized by the
  ...
  always-valid confidence sequences (mSPRT form). Small biddable tests can use group sequential
...
- **Interference.** When arms share line-item budgets or pacing, per-impression randomization
  leaks between arms. Pacing and bid-shading tests should randomize by biddable, and Studio's
...
- **Experiment reporting is spread across hand-built Omni dashboards** in the outcomes-tests
  folder and a few others. Examples: the VSR model test, CTV attention tests, programmatic VCR
  tests, direct CTR version comparisons, bidding-strategy line tests, OBC model diagnostics.
  Four of them contain near-identical "control vs test" tiles with hard-coded deal or line filters.
- **No Omni topic models experiment assignments.** Every dashboard rebuilds the split from
  auction facts.
...
- **Kraken auction data arrives in Omni about two days late,** so early ramps rely on Grafana.
...
| **Omni** | The semantic model over the `exp` tables, the standard experiment dashboard, the portfolio view, ad-hoc slicing, history. |
| **Grafana and Monte Carlo** | Hourly and real-time monitoring and alerting. Omni data lags, so alerts never depend on it. |
...
5. **Readout export.** A decision block with hypothesis, primary and guardrail results and
   ship, extend or kill, exported as Markdown for Confluence, with a stable link to the Omni
   dashboard.
...
- **A portfolio view** listing live experiments, owners, exposure, days running and decision
  status, rolled up the way the outcomes business-impact dashboard already reports lift.
...
| New version into the ML client | MLE engineer | Model code in panamax, then a panamax release, subject to code freezes | direct-ctr v104: pipeline ready 7/10, panamax release 9/24 |
| Deploy to kraken | SSP engineer | Bump panamax, deploy. The routing policy only loads at startup, so pods must roll. | v104: about 11 weeks end to end, still at 0% |
| Monitors | MLE engineer | Grafana and Monte Carlo alerts built by hand per release | about a sprint |
| Route traffic | SSP engineer only | Flag gates keyed on ad slot, plus the ML client's allocation percentage, plus the JSON weights flag | hours to days per step; "about 10 minutes to propagate" |
| Ramp | product manager | 1% → 5% → 15% → 30% → 50% → 75% | 2 to 10 weeks |
| Compare | data science, analytics | Per-test Omni and Grafana dashboards, offline notebooks | days; Omni lags 2 days |
...
4. **Results cannot be attributed to a model version.** Flag exposures are never logged in
   kraken, the predictor event is 1% sampled with no bid ID, and some paths hardcode the model
...
- **Guardrails with alerting, then auto-rollback.** Thresholds such as a calibration-ratio limit
  or a margin drop versus control page the owner first. Once trusted, a breach reverts the
  commit automatically.
- **MLE-scoped permissions.** MLE and data science edit only the model-routing file, self-serve
  up to a ramp cap (for example 10%). Above the cap needs a second approver from SSP or
...
model-routing.directctr:
  metadata:
    ...
    experiment:
      unit: {type: entity, key: biddableId}
  ...
  targeting:
    - name: ctv-test-lines
      query: auctionType eq "direct" and biddableId in ["282898", "282954"]
      variation: v104
  defaultRule: {variation: champion}
```

Kraken changes, by file:
...
- `internal/predictor/predictor.go` and the panamax predictor: preload every version in the
...
Sizes: S is under 2 days, M is under a week, L is over a week. "Service" means work in kraken or
scylla, "Data" means arion and Snowflake, "Studio" means this repo, "Config" needs no code.
...
- **Reporting:** R1 and R2. The Omni topic and one template dashboard.
...
| --- | --- | --- | --- | --- | --- |
| F1 | P0 | **GO Feature Flag implementation of the `Experimenter` interface** | Service | M | New package next to the existing one, e.g. `internal/experiment/goff/`. Map the subject key to `targetingKey` and the subject attributes to the evaluation context. Implement it with the shared `kargo-flags` package (section 2). Choose the provider with one env var (`FLAG_PROVIDER=goff\|current`) so rollback is a config flip. Keep the existing Prometheus counters (`experimentActive`, `experimentGetCount`). |
| F2 | P0 | **One-time flag import** | Script (in the flags repo) | M | Convert the current server-side flag configuration export (flags, variations, targeting rules, splits) into `*.goff.yaml` files: one file per owning team, per environment. Each generated file is reviewed in a PR. Import all 73 flags, stale ones included, because some are still read by code. A flag that is switched off maps to `disable: true`, which returns the code default exactly as today. Per-entity override rules map one-to-one to targeting rules. Experiment allocations, including pass-through rules, carry their salts and ranges over exactly (X2). |
...
| F5 | P0 | **Slack change feed with zero code** | Config | S | Subscribe the GitHub Slack app to commits on the flags repo's `main` in the existing change channels. Studio commit messages already read like the current notifications, e.g. `[production] kraken/tmax: disabled`, with the author, so the channel stays the incident timeline. One difference: this posts staging changes too, whereas only production changes are announced today. Accept the extra noise until F9, or subscribe only in the second channel. |
...
| F8 | P1 | **Team-scoped permissions** | Config | S | Split files and Okta groups by owning team. Examples: `kraken.goff.yaml` (group `flags-kraken`), `scylla.goff.yaml` (`flags-scylla`), `models.goff.yaml` (`flags-mle`), and business users with `toggle` and `rollout` only on their file in `production`. The permission model already supports this; it only needs the files split and the groups created in Okta. |
| F9 | P1 | **Native Slack notifications with per-team routing** | Studio | M | After a successful commit, post to a Slack incoming webhook chosen by the team file (config: `notifications: [{match: "kraken", webhook: ...}]`). Include the plain-language summary, the environment, the author, and links to the flag and the commit. This fixes the "every team sees every change" noise. Hook point: after the write in `internal/server/service.go`. Webhook URLs are secrets. |
| F10 | P1 | **Change reason on save** | Studio | S | Optional free-text "why" in the review dialog, written into the commit trailer and the Slack message. It replaces the habit of replying "intentional" in the notification thread. |
...
| F13 | P1 | **JSON value validation** | Studio | M | Many flags carry large JSON configs (bidding strategy, OpenRTB overrides, model weights). Add an optional JSON Schema per flag (`metadata.schema` pointing at a file in the flags repo), validated in the variations editor and on the server before commit. |
...
| F17 | P1 | **Restore a previous version** | Studio | M | A "Restore" action on each history entry that rewrites the flag to its state at that commit, through the normal review dialog. Per-flag history exists, but reverting today means a hand edit. |
...
| F23 | P2 | **Two-person approval for protected environments** | Studio | L | Optional mode that opens a PR instead of committing directly for chosen environments or teams. Only worth building if F7 and F12 prove insufficient. |
...
permissions:
  ...
  - group: "*"                 # any signed-in Kargo user can read
    allow: ["*"]
    actions: [view]
```
...
| # | Priority | Feature | Where | Size | Notes |
| --- | --- | --- | --- | --- | --- |
| A1 | P0 | **Exposure events from the new evaluator** | Service (scylla, then kraken) | S | Log only exposed subjects, with the allocation key from the evaluator and the existing experiment-key naming (`flag-allocation`), through the existing event path. The current assignment table keeps filling during cutover. Kraken starts logging exposures for the first time. |
...
| A14 | P2 | **Video-player analysis path** | Data | M | A `video_player` entity reading the Fabrik impression tables, with the video-player metrics. None of these tests has run since 2025, so this waits for demand. |

### Reporting

| # | Priority | Feature | Where | Size | Notes |
| --- | --- | --- | --- | --- | --- |
| R1 | P0 | **Omni "Experiments" topic** | Omni | M | Over `exp_results`, `exp_suffstats`, `exp_registry`, `flag_change_log` and exposures joined to model predictor events, with the standard measures in section 5. |
| R2 | P0 | **Experiment dashboard template** | Omni | M | One verified, parameterized dashboard with the tiles in section 5, 3-month rolling and year-over-year on every time series. Replaces the per-test copies. |
| R3 | P1 | **Flag change log table** | Data | S | Built from flags-repo commits. Feeds annotations in Studio and Omni and incident timelines. |
...
| R6 | P1 | **Studio experiment set-up** | Studio | M | Section 5, view 2. Writes `experiments/*.yaml` and the flag's experiment block in one reviewed commit. |
...
| R11 | P2 | **Omni portfolio view** | Omni | S | Cross-experiment roll-up of lift and decisions. |

### Model testing

| # | Priority | Feature | Where | Size | Notes |
| --- | --- | --- | --- | --- | --- |
| M1 | P0 | **Weighted JSON model routing on the new provider** | Service (kraken) | S | Keep the same JSON shape so no model config changes. Register a provider-neutral scheme (e.g. `flag://kraken_ctr_model`) alongside the current scheme, and switch the env var per deployment. |
| M2 | P1 | **Pin model versions per request** | Service (kraken, panamax) | M | Preload every version in the policy, hot-reload the policy, pin the chosen version on each predict call. Removes pod restarts from routing changes. |
| M3 | P1 | **Model-routing flag kind and catalog** | Studio | M | Version picker from S3, MLflow and panamax families, with an artifact-and-load check per region. A catalog screen lists policies, versions, load status, last release and whether monitors exist. |
| M4 | P1 | **Sticky routing by biddable** | Service | S | Route through the split evaluator keyed on biddable by default. Depends on X1. |
| M5 | P1 | **Model exposure logging** | Service (kraken) | M | Bid ID, flag, variation, mode and version on every bid, with the version label set on every path. |
...
| M8 | P2 | **Automatic rollback on guardrail breach** | Studio + Service | M | Reverts the routing commit and pages, once the alerts in M6 have proven reliable. |
...
kraken, with data science part-time on validation. The P0 work is roughly 14 engineer-weeks.

| Weeks | Flags | Split testing | Analysis and reporting | Model testing |
| --- | --- | --- | --- | --- |
| 1–2 | Private flags repo, GitHub App, S3 sync, Okta app, Studio deploy (F3–F6) | Pull one config and confirm the shard layout; build X1 with the golden test | Ship the dedup fix (A2). Write the registry and metric catalog (A6, A7) | — |
...
| 4 | Cut over kraken: staging canary, one region, all regions. Rollback is the provider env var | — | Build A5. Kraken starts logging exposures (A1) | M1 on the new provider |
| 5 | Cut over scylla the same way. In-progress ramps keep their assignments | — | Confirm exposures keep arriving with the expected variants | — |
| 5–7 | Flag edits happen only in Studio. Old UI read-only | — | Validation (A8). Omni topic and template (R1, R2) | — |
...
| Per-impression tests leak through shared budgets | Pacing and shading tests randomize by biddable, and set-up warns when the unit and the treatment don't fit. |
| S3 sync lag | Changes still go live within about a minute (Action plus 60-second poll), comparable to today. Studio's "live in about N seconds" message is set to match. |
| GitHub outage blocks edits | Break-glass: edit the S3 object directly, then reconcile the repo afterwards. Document this in the runbook. |
| Import mistranslates a rule | Per-flag PR review, preview checks and zero-mismatch shadow evaluation on every flag. |
| Studio is a young project | Small codebase with an end-to-end suite. Kargo can patch the fork and send fixes upstream. |
